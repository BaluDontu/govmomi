...
func TestClient(t *testing.T) {
	...
	createSpecVSAN, err := CreatePbmCapabilityProfileSpec(pbmCreateSpecForVSAN)
	if err != nil {
		t.Fatal(err)
	}

	// 5. Create SPBM VSAN profile.
	vsanProfileID, err := pc.CreatePbmProfile(ctx, createSpecVSAN)
	if err != nil {
		t.Fatal(err)
	}
	...
	compatibleDatastores, err := pc.GetCompatibleDatastores(ctx, datastores, nil, []types.PbmProfileId{vsanProfileID})
	if err != nil {
		t.Fatal(err)
	}
	...
	nonCompatibleDatastores, err := pc.GetNonCompatibleDatastores(ctx, datastores, nil, []types.PbmProfileId{vsanProfileID})
	if err != nil {
		t.Fatal(err)
	}
	...
	// Create PBM capability spec for the above defined user spec.
	createSpecVSANandSIOC, err := CreatePbmCapabilityProfileSpec(pbmCreateSpecVSANandSIOC)
	if err != nil {
		t.Fatal(err)
	}

	// 9. Create SPBM VSAN profile.
	vsansiocProfileID, err := pc.CreatePbmProfile(ctx, createSpecVSANandSIOC)
	if err != nil {
		t.Fatal(err)
	}
	...
	profileID, err := pc.GetPbmProfileIDByName(ctx, "Kubernetes-VSAN-SIOC-TestPolicy")
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = pc.DeletePbmProfile(ctx, []types.PbmProfileId{vsanProfileID, vsansiocProfileID})
	if err != nil {
		t.Fatal(err)
	}
